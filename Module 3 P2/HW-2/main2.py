...
class Catalog:
    ...
    def removeCourse(self, cid):
      del self.courseOfferings[cid] #delets the course
...
class Student(Person):
    ...
    def enrollCourse(self, cid, catalog, semester):
      if self.active == False or self.hold == True:
        return "Unsuccessful operation"
      elif cid not in catalog.courseOfferings.keys():
        return "Course not found"
      elif cid in self.semesters[semester].courses.keys() :
        return "Course already enrolled"
      else:
        object = self.semesters.get(semester)
        course = catalog.courseOfferings[cid][0]
        object.addCourse(course)
        print("object",object)
        print("course",course)
        print("sem",self.semesters)
        print("coursesd",self.semesters[semester].courses)
    ...
    def dropCourse(self, cid):
      if self.active == False or self.hold == True:
        return "Unsuccessful operation"
      elif cid not in self.semesters[len(self.semesters.keys())].courses.keys():
        print("sdfsd",len(self.semesters.keys()))
        return "Course not found"
      else:
        course = self.semesters[len(self.semesters.keys())].courses.pop(cid)
