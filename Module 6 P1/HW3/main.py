...
class Calculator:
    ...
    def calculate(self):
        ...
        a = Calculator()._getPostfix(self.__expr)
        oper = ["+","-","*","%","/","^","(",")"]
        fix = []
        level = {")":1,"(":1,"+":2,"-":2,"/":3,"*":3,"^":4}
      
        for item in a: #get the result from the calculator
          fix.append(item)

        
        for i in (0,len(fix)): #determine +,-,*,/
          number1 = fix[i]
          number2 = fix[i+1]
          operator = fix[i+2]
            
          if operator == "+": #when adding numbers
            return number2 + number1
            
          elif operator == "-": #when subtracting numbers
            return number2 - number1

          elif operator == "*": #when multiplying numbers
            return number1 * number2

          elif operator == "/": #when dividing numbers
            return number2 / number1
...
class AdvancedCalculator:
    ...
    def _isVariable(self, word): #check if it's a valid name
        '''
            ...
            False
        '''
        if word.isalpha() == True and isinstance(word,str):
          return True
        else:
          return False
